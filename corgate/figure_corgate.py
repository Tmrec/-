...
if "property" =="property":
    ...
    name0_list      = ["","","","","","","",""]
    color_list      = ["green","red","red","orange","yellow","black","blue","red","orange","yellow","black","blue","red","orange","yellow","black"]
    ct_ylim         = "off"#[0,1]  #"off"#
    cl_ylim         = [-0.1,0.8]#"off"#[-25,15] #"off"
    cd_ylim         = [-0.1,0.8]
    cl_ylim         = [-0.1,0.8]#[0,1]#"off"#[-25,15] #"off"
    cd_ylim         = [-0.1,0.8]#"off"
    grid            = False
    transParent     = True
    line0           = "off"
...
def makeFigure(fontSize,style,root_dir_name,k_list,s_list,number_of_data,test_fig,freq,alphaU_list,comparedata,k,s):
    ...
    if uList_changer =="on":
        print(alphaU_list)
        change_ulist=[]
        if takeUlistname in k:
            print(k)
            if takeUlistname in k:
                au = k.split(takeUlistname)[1]
                for i in range(len(alphaU_list)):
                    change_ulist.append(int(au))
        elif takeUlistname in k_list[0]:
            for i in range(len(k_list)):
                au =  k_list[i].split(takeUlistname)[1]
                change_ulist.append(int(au))
            
            #print(k)
            #print(s)
        if len(change_ulist)!=0:
            alphaU_list = change_ulist
    ...
    if "calculate"=="calculate":
        ...
        for i in range(number_of_data):
            ...
            print(k_list[i])
            print(np.degrees(alphaU_list[i]))
    ...
    if "output"=="output":
        ...
        if "print"=="print":
            ...
            for i in range(number_of_data):
                ...
                cdMean_list.append(np.mean(cd_list_all[i]))

            Mean_list =[]
            Mean_list.append(clMean_list)
            Mean_list.append(cdMean_list)
...
if "input" == "input":
    ...
    alphaU_list     = [0,2,4,6,8,10,12,15,17,20]
    comparedata     = ""
    fontSize        = 15
    fontSizelabal   = 20
    style           = "o" #o -o --o
...
if "calc"=="calc":  
    ...
    if "fig_k_s"=="fig_k_s":
        ...
        for i in range(len(ctMean2_list)):
            plt.plot(alphaU_list, ctMean2_list[i],style,markersize=10.0,linewidth=4.0,label=s_list_save[i],color=color_list[0])
        for i in range(len(ctMean2_list_second)):
            if TopFolder2 != "":
                plt.plot(alphaU_list, ctMean2_list_second[i],style,linewidth=4.0,label=s_list_save2[i])
        plt.legend(fontsize=legendFont)
        plt.xlabel('$\it{α[deg]}$',fontsize = fontSize)
        plt.ylabel('$\it{C_d}$',fontsize = fontSize)
        figMean1.savefig("./0_fig_using/"+root_dir_name+"/cdMean_a.png",transparent=transParent)
        ...
        for i in range(len(clMean2_list)):
            plt.plot(alphaU_list, clMean2_list[i],style,markersize=10.0,linewidth=4.0,label=s_list_save[i],color=color_list[1])

        for i in range(len(clMean2_list_second)):
            if TopFolder2 != "":
                plt.plot(alphaU_list, clMean2_list_second[i],style,markersize=15.0,linewidth=4.0,label=s_list_save2[i])
            
        plt.legend(fontsize=legendFont)
        if cl_ylim != "off":
            plt.ylim(cl_ylim)
        plt.xlabel('$\it{α[deg]}$',fontsize = fontSize)
        plt.ylabel('$\it{Cl,_{mean}}$',fontsize = fontSize)
        figMeanCl1.savefig("./0_fig_using/"+root_dir_name+"/clMean_a.png",transparent=transParent)
